...
# Filter nasicon-type electrolytes
nasicon = df[df['family'] == 'NASICON']

# Filter room temperature measurements (15-35°C) to isolate dopant effects
nasicon_rt = nasicon[nasicon['temperature'].between(15, 35)].copy()

print(f"using data: {len(nasicon_rt)}")
...
ax.set_aspect('equal')
plt.tight_layout()
plt.savefig('figures/nasicon_loocv_parity.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig("figures/nasicon_shap_beeswarm.png", dpi=150)
plt.close()

# Save bar plot
shap.summary_plot(shap_values, X, plot_type="bar", show=False)
plt.tight_layout()
plt.savefig("figures/nasicon_shap-importance.png", dpi=150)
plt.close()
